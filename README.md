...
1. Create a builder resource. This resource tracks a builder on registry and will rebuild images when the builder has updated buildpacks. 
    Two types of builders can be create, Cluster Scoped Builders and Namespace Scoped Builders
   ...
   1. Cluster Scoped Builders
        ...
        spec:
          ...
          # - name: builder-secret
        ```
   **Note:** Make sure to create the imagePullSecrets if your Builder is hosted
   in a registry that is not public. These secrets are similar to all the other
   imagePullSecrets from Kubernetes
